...
.addField(`Music [${slash.filter((m) => m.category === "Music").size}]:`,`\`\`\`${slash.filter((m) => m.category === "Music").map((m) => `${m.name}`).join(" , ")}\`\`\``)
.addField(`Developer [${slash.filter((m) => m.category === "Developer").size}]:`,`\`\`\`${slash.filter((m) => m.category === "Developer").map((m) => `${m.name}`).join(" , ")}\`\`\``)
.setTimestamp()
